(() => {
  ...
  const rockets = [], particles = [], flashes = [], smoke = [], ripples = [];
  const palettes = {
  ...
  class Particle{
    constructor(x,y,vx,vy,color,life,size,gravity=.045,drag=.985,twinkle=false){
      Object.assign(this,{x,y,vx,vy,color,life,maxLife:life,size,gravity,drag,twinkle});this.prevX=x;this.prevY=y;this.seed=Math.random()*20;
    }
    update(dt){this.prevX=this.x;this.prevY=this.y;this.vx*=Math.pow(this.drag,dt);this.vy*=Math.pow(this.drag,dt);this.vy+=this.gravity*dt;this.x+=this.vx*dt;this.y+=this.vy*dt;this.life-=dt;}
    draw(){
      ...
      ctx.strokeStyle=rgba(this.color,a*.9);ctx.lineWidth=this.size*Math.max(.35,a);ctx.stroke();
    }
  }

  function explode(x,y,type,colorKey){
    const cols=palettes[colorKey], mobile=W<700, quality=mobile?.5:.72;
    let count=type==='willow'?150:type==='ring'?110:type==='peony'?180:210; count*=quality;
    flashes.push({x,y,r:8,life:18,max:18,color:cols[1]});
  ...
  function render(now){
    ...
    for(let i=flashes.length-1;i>=0;i--){const f=flashes[i];f.life-=dt;f.r+=7*dt;const a=Math.max(0,f.life/f.max);const gr=ctx.createRadialGradient(f.x,f.y,0,f.x,f.y,f.r);gr.addColorStop(0,rgba('#ffffff',a*.7));gr.addColorStop(.15,rgba(f.color,a*.35));gr.addColorStop(1,rgba(f.color,0));ctx.fillStyle=gr;ctx.fillRect(f.x-f.r,f.y-f.r,f.r*2,f.r*2);if(f.life<=0)flashes.splice(i,1)}
  ...
  document.getElementById('enterBtn').onclick=()=>{initAudio();if(audio.state==='suspended')audio.resume();document.getElementById('startScreen').classList.add('hidden');setTimeout(()=>{launch(W*.64,H*.3,'chrysanthemum','gold');setTimeout(()=>launch(W*.76,H*.22,'willow','sakura'),650)},600)};
})();
